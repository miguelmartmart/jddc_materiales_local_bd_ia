...
class TestPhase4SIUOFeedback:
    def test_register_feedback_called(self):
        ...
        with patch(
            "backend.modules.chat.deep_analysis.phase4.get_context_retriever",
...
class TestHelpers:
    ...
    def test_build_warnings_html_with_warnings(self):
        """_build_warnings_html genera Markdown puro (NO HTML con divs)."""
        agent = make_agent()
        result = EpicAnalysisResult(question="test", depth=AnalysisDepth.EPIC)
        result.warnings = ["Advertencia 1", "Advertencia 2"]
        result.anomalies = ["Anomalía detectada"]
        output = agent._build_warnings_html(result)
        assert "Advertencia 1" in output
        assert "Anomalía detectada" in output
        # NO debe contener HTML crudo
