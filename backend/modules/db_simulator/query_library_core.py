...
QUERY_LIBRARY: List[Dict[str, Any]] = [
...

    {
        ...
        "desc_simple": (
            "Suma del campo IMPORTETOTAL de todos los documentos con TIPO=13 (facturas de venta). "
            "Incluye todas las facturas emitidas a clientes en el período disponible en la base de datos."
        ),
        "desc_tecnica": (
            ...
            "El campo IMPORTETOTAL incluye IVA: verificable comparando IMPORTEBASE + cuota IVA con IMPORTETOTAL "
            "(ver panel de evidencia IVA en las verificaciones). "
            "Para obtener la base imponible sin IVA, usar el campo IMPORTEBASE de DOCCAB."
        ),
    ...
    },
    {
        ...
        "desc_tecnica": (
            "AVG(IMPORTETOTAL) sobre facturas TIPO=13 en DOCCAB. "
            "También devuelve MIN y MAX para conocer el rango de importes. "
            "El ticket medio varía según el tipo de trabajo registrado en las líneas (DOCLIN). "
            "Para desglosar por tipo de trabajo, cruzar con DOCLIN.CODARTICULO y ARTICULO.CODFAMILIA."
        ),
    ...
    },
    {
        ...
        "desc_simple": (
            "Los 10 clientes con mayor IMPORTETOTAL acumulado en facturas TIPO=13. "
            "Incluye el porcentaje que representa cada cliente sobre el total facturado. "
            "Permite identificar la concentración de ingresos en la cartera de clientes."
        ),
        "desc_tecnica": (
            "JOIN DOCCAB-CLIENTE agrupando por cliente, sumando IMPORTETOTAL de TIPO=13. "
            "Incluye el porcentaje sobre el total (PCT_TOTAL) para calcular el índice de concentración. "
            "PCT_TOTAL = SUM(IMPORTETOTAL_cliente) / SUM(IMPORTETOTAL_total) * 100. "
            "Si PCT_TOTAL acumulado de los 3 primeros supera el 50%, la facturación está concentrada en pocos clientes."
        ),
        "sql": (
            "SELECT COALESCE(C.NOMBRECOMERCIAL, C.RAZONSOCIAL, 'Sin nombre') AS NOMBRE, COUNT(D.CODIGO) AS N_FACTURAS, "
            ...
            "WHERE D.TIPO=13 "
            "GROUP BY C.CODIGO, C.NOMBRECOMERCIAL, C.RAZONSOCIAL "
    ...
    },
    {
        ...
        "sql": (
            "SELECT "
            "SUM(CASE WHEN TIPO=0 THEN 1 ELSE 0 END) AS PRESUPUESTOS, "
            "SUM(CASE WHEN TIPO=13 THEN 1 ELSE 0 END) AS FACTURAS, "
            "CASE WHEN SUM(CASE WHEN TIPO=0 THEN 1 ELSE 0 END)=0 THEN 0.0 "
            "ELSE ROUND(SUM(CASE WHEN TIPO=13 THEN 1.0 ELSE 0 END)*100.0/"
            "SUM(CASE WHEN TIPO=0 THEN 1.0 ELSE 0 END),2) END AS TASA_CONVERSION_PCT "
            "FROM DOCCAB WHERE TIPO IN (0,13)"
        ),
        "dept": [Dept.VENTAS, Dept.DIRECCION],
        "rol": [Rol.DIRECTOR, Rol.GERENTE, Rol.COMERCIAL],
        "tipo": Tipo.KPI,
    ...
    },
    {
        ...
        "sql": (
            "SELECT COALESCE(C.CODFORMAPAGO,'Sin especificar') AS FORMA_PAGO, COUNT(D.CODIGO) AS N_FACTURAS, "
            "ROUND(SUM(D.IMPORTETOTAL),2) AS TOTAL "
            "FROM DOCCAB D JOIN CLIENTE C ON C.CODIGO=D.CODCLIENTE "
            "WHERE D.TIPO=13 GROUP BY C.CODFORMAPAGO ORDER BY TOTAL DESC"
        ),
    ...
    },
    {
        ...
        "sql": (
            "SELECT COALESCE(C.NOMBRECOMERCIAL, C.RAZONSOCIAL, 'Sin nombre'), C.TEL, "
            "MAX(D.FECHA) AS ULTIMA_COMPRA "
            "FROM CLIENTE C "
            "LEFT JOIN DOCCAB D ON D.CODCLIENTE=C.CODIGO AND D.TIPO=13 "
            "GROUP BY C.CODIGO, C.TEL "
    ...
    },
    {
        ...
        "sql": (
            "SELECT COALESCE(C.NOMBRECOMERCIAL, C.RAZONSOCIAL, 'Sin nombre'), C.TEL, MIN(D.FECHA) AS PRIMERA_COMPRA, "
    ...
    },
    {
        ...
        "desc_tecnica": (
            "JOIN DOCCAB-CLIENTE agrupando por CODIGO_POSTAL. "
        ...
        "sql": (
            "SELECT COALESCE(C.CP,'Sin especificar') AS CODIGO_POSTAL, COUNT(D.CODIGO) AS N_FACTURAS, "
            "ROUND(SUM(D.IMPORTETOTAL),2) AS TOTAL "
            "FROM DOCCAB D JOIN CLIENTE C ON C.CODIGO=D.CODCLIENTE "
            "WHERE D.TIPO=13 "
            "GROUP BY C.CP ORDER BY TOTAL DESC LIMIT 15"
        ),
    ...
    },
    {
        ...
        "sql": (
            "SELECT D.FECHA, COALESCE(C.NOMBRECOMERCIAL, C.RAZONSOCIAL, 'Sin nombre'), C.TEL, ROUND(D.IMPORTETOTAL,2) AS IMPORTE, "
...

    {
        ...
        "sql": (
            ...
            "FROM DOCCAB D JOIN CLIENTE C ON C.CODIGO=D.CODCLIENTE "
            "WHERE D.TIPO=13 GROUP BY C.CODIGO ORDER BY TOTAL DESC LIMIT 5)"
        ),
    ...
    },
    {
        ...
        "sql": (
            "SELECT COALESCE(C.NOMBRECOMERCIAL, C.RAZONSOCIAL, 'Sin nombre'), COUNT(DISTINCT L.CODARTICULO) AS N_PRODUCTOS_DISTINTOS, "
            "COUNT(L.CODIGO) AS N_LINEAS "
            "FROM CLIENTE C "
    ...
    },
    {
        ...
        "sql": (
            "SELECT COALESCE(C.NOMBRECOMERCIAL, C.RAZONSOCIAL, 'Sin nombre'), C.TEL, COUNT(D.CODIGO) AS N_FACTURAS_ANTIGUAS, "
    ...
    },
    {
        ...
        "sql": (
            ...
            "ROUND(SUM(CASE WHEN D.FECHA >= date('now','-30 days') THEN ROUND(CAST(L.CANTIDAD AS REAL)*CAST(L.PRECIO AS REAL),2) ELSE 0 END),2) AS IMP_ACTUAL "
            "FROM DOCLIN L "
...

    {
        ...
        "sql": (
            "SELECT COALESCE(P.NOMBRECOMERCIAL, P.RAZONSOCIAL, 'Sin nombre'), COUNT(A.CODIGO) AS N_ARTICULOS, "
            "ROUND(AVG(A.PRECIOVENTA),2) AS PRECIO_MEDIO "
            "FROM ARTICULO A JOIN PROVEED P ON P.CODIGO=A.PROVEEDDEFECTO "
            "GROUP BY P.CODIGO, COALESCE(P.NOMBRECOMERCIAL, P.RAZONSOCIAL, 'Sin nombre') ORDER BY N_ARTICULOS DESC LIMIT 10"
        ),
    ...
    },
    {
        ...
        "desc_tecnica": (
            "ARTICULO con PROVEEDDEFECTO=0 o NULL. "
            "En JDDC, PROVEEDDEFECTO=0 es el valor por defecto cuando no se ha asignado proveedor. "
        ...
        "sql": (
            "SELECT CODIGO, NOMBRE, PRECIOVENTA, STOCKARTICULO "
            "FROM ARTICULO WHERE PROVEEDDEFECTO=0 OR PROVEEDDEFECTO IS NULL "
    ...
    },
    {
        ...
        "sql": (
            "SELECT COALESCE(P.NOMBRECOMERCIAL, P.RAZONSOCIAL, 'Sin nombre') AS PROVEED, COUNT(A.CODIGO) AS N_REFERENCIAS, "
            "ROUND(MIN(A.PRECIOVENTA),2) AS PRECIO_MIN, ROUND(MAX(A.PRECIOVENTA),2) AS PRECIO_MAX "
            "FROM PROVEED P LEFT JOIN ARTICULO A ON A.PROVEEDDEFECTO=P.CODIGO "
            "GROUP BY P.CODIGO, COALESCE(P.NOMBRECOMERCIAL, P.RAZONSOCIAL, 'Sin nombre') ORDER BY N_REFERENCIAS DESC"
        ),
        "dept": [Dept.COMPRAS],
        "rol": [Rol.GERENTE, Rol.ADMIN],
        "tipo": Tipo.OPERACIONAL,
    ...
    },
    {
        ...
        "sql": (
            "SELECT A.NOMBRE, COALESCE(P.NOMBRECOMERCIAL, P.RAZONSOCIAL, 'Sin nombre') AS PROVEED, A.STOCKARTICULO AS STOCK, A.PRECIOVENTA "
            "FROM ARTICULO A JOIN PROVEED P ON P.CODIGO=A.PROVEEDDEFECTO "
    ...
    },
    {
        ...
        "sql": (
            "SELECT A.NOMBRE, COALESCE(P.NOMBRECOMERCIAL, P.RAZONSOCIAL, 'Sin nombre') AS PROVEED, "
            "SUM(L.CANTIDAD) AS TOTAL_VENDIDO, "
            "ROUND(SUM(CAST(L.CANTIDAD AS REAL)*CAST(L.PRECIO AS REAL)),2) AS IMPORTE_TOTAL, "
            "A.PRECIOVENTA AS PRECIO_ACTUAL "
            "FROM DOCLIN L "
            "JOIN ARTICULO A ON L.CODARTICULO = A.CODIGO "
            "JOIN PROVEED P ON P.CODIGO=A.PROVEEDDEFECTO "
            "JOIN DOCCAB D ON D.CODIGO = L.CODDOCUMENTO AND D.TIPO=13 "
            "GROUP BY A.CODIGO, A.NOMBRE, COALESCE(P.NOMBRECOMERCIAL, P.RAZONSOCIAL, 'Sin nombre'), A.PRECIOVENTA "
    ...
    },
    {
        ...
        "sql": (
            ...
            "ROUND(AVG(A.PRECIOVENTA),2) AS PRECIO_MEDIO_CATALOGO "
    ...
    },
    {
        ...
        "desc_tecnica": (
            "ORDER BY PRECIOVENTA DESC en ARTICULO. "
        ...
        "sql": (
            "SELECT A.NOMBRE, F.NOMBRE AS FAMILIA, COALESCE(P.NOMBRECOMERCIAL, P.RAZONSOCIAL, 'Sin nombre') AS PROVEED, "
            "A.PRECIOVENTA, A.STOCKARTICULO "
            "FROM ARTICULO A "
...

    {
        ...
        "desc_tecnica": (
            "SUM(STOCKARTICULO * PRECIOVENTA) en ARTICULO. "
        ...
        "sql": (
            ...
            "ROUND(SUM(STOCKARTICULO*PRECIOVENTA),2) AS VALOR_STOCK_TOTAL "
    ...
    },
    {
        ...
        "sql": (
            "SELECT A.NOMBRE, F.NOMBRE AS FAMILIA, A.PRECIOVENTA, A.STOCKARTICULO "
    ...
    },
    {
        ...
        "sql": (
            "SELECT A.NOMBRE, F.NOMBRE AS FAMILIA, COALESCE(P.NOMBRECOMERCIAL, P.RAZONSOCIAL, 'Sin nombre') AS PROVEED, "
            "A.STOCKARTICULO AS STOCK, A.PRECIOVENTA "
            "FROM ARTICULO A "
            "LEFT JOIN FAMILIA F ON F.CODIGO=A.CODFAMILIA "
            "LEFT JOIN PROVEED P ON P.CODIGO=A.PROVEEDDEFECTO "
    ...
    },
    {
        ...
        "sql": (
            ...
            "ROUND(SUM(A.STOCKARTICULO),2) AS CANTIDAD, "
            "ROUND(SUM(A.STOCKARTICULO*A.PRECIOVENTA),2) AS VALOR "
            "FROM ARTICULO A LEFT JOIN FAMILIA F ON F.CODIGO=A.CODFAMILIA "
    ...
    },
    {
        ...
        "sql": (
            "SELECT A.NOMBRE, A.STOCKARTICULO AS STOCK, A.PRECIOVENTA, "
            "ROUND(A.STOCKARTICULO*A.PRECIOVENTA,2) AS VALOR_INMOVILIZADO "
            "FROM ARTICULO A "
            "WHERE A.STOCKARTICULO > 0 "
            "AND A.CODIGO NOT IN ("
            "SELECT DISTINCT CAST(L.CODARTICULO AS INTEGER) FROM DOCLIN L "
            "JOIN DOCCAB D ON D.CODIGO = L.CODDOCUMENTO AND D.TIPO=13 "
    ...
    },
    {
        ...
        "sql": (
            ...
            "ELSE A.STOCKARTICULO END)*A.PRECIOVENTA, 2) AS VALOR_EXCESO "
            "FROM ARTICULO A "
            "LEFT JOIN DOCLIN L ON L.CODARTICULO = A.CODIGO "
            "LEFT JOIN DOCCAB D ON D.CODIGO = L.CODDOCUMENTO AND D.TIPO=13 "
            "AND D.FECHA >= date('now','-90 days') "
            "GROUP BY A.CODIGO, A.NOMBRE, A.STOCKARTICULO, A.PRECIOVENTA "
    ...
    },
    {
        ...
        "sql": (
            "SELECT E.FECHA, A.NOMBRE AS ARTICULO, E.IMPCOSTE AS CANTIDAD, E.IMPCOSTE, E.IMPVENTA "
            "FROM ESTALMACEN E JOIN ARTICULO A ON A.CODIGO=E.CODIGO "
    ...
    },
    {
        ...
        "sql": (
            ...
            "ROUND(SUM(CAST(L.CANTIDAD AS REAL)*CAST(L.PRECIO AS REAL)),2) AS TOTAL_IMPORTE, "
    ...
    },
    {
        ...
        "desc_tecnica": (
            "SUM(ROUND(CAST(CANTIDAD AS REAL)*CAST(PRECIOVENTA AS REAL),2)) en CAJA TIPO=1 del mes actual. "
    ...
    },
    {
        ...
        "desc_tecnica": (
            "SUM(ROUND(CAST(CANTIDAD AS REAL)*CAST(PRECIOVENTA AS REAL),2)) en CAJA TIPO=2 del mes actual. "
    ...
    },
    {
        ...
        "sql": (
            "SELECT A.NOMBRE, A.PRECIOVENTA AS PVP, "
            "ROUND(AVG(E.IMPCOSTE),2) AS COSTE_MEDIO, "
            "ROUND(A.PRECIOVENTA - AVG(E.IMPCOSTE),2) AS MARGEN_BRUTO, "
            "ROUND((A.PRECIOVENTA - AVG(E.IMPCOSTE))*100.0/NULLIF(A.PRECIOVENTA,0),1) AS MARGEN_PCT "
            "FROM ARTICULO A "
            "JOIN ESTALMACEN E ON E.CODIGO=A.CODIGO "
            "WHERE E.IMPCOSTE > 0 AND A.PRECIOVENTA > 0 "
    ...
    },
    {
        ...
        "sql": (
            "SELECT D.FECHA, COALESCE(C.NOMBRECOMERCIAL, C.RAZONSOCIAL, 'Sin nombre') AS CLIENTE, C.TEL, ROUND(D.IMPORTETOTAL,2) AS IMPORTE, "
            ...
            "ORDER BY D.IMPORTETOTAL DESC LIMIT 15"
        ),
        "dept": [Dept.FINANZAS, Dept.VENTAS],
        "rol": [Rol.GERENTE, Rol.ADMIN],
        "tipo": Tipo.RIESGO,
    ...
    },
    {
        ...
        "sql": (
            "SELECT A.NOMBRE, L.DESCUENTOS, L.PRECIO, ROUND(CAST(L.CANTIDAD AS REAL)*CAST(L.PRECIO AS REAL),2) AS IMPORTE_CON_DTO, "
            "ROUND(CAST(L.CANTIDAD AS REAL)*CAST(L.PRECIO AS REAL),2) AS SIN_DESCUENTO, "
            "ROUND(CAST(L.CANTIDAD AS REAL)*CAST(L.PRECIO AS REAL)*CAST(L.DESCUENTOS AS REAL)/100.0,2) AS DESCUENTO_APLICADO "
            "FROM DOCLIN L "
            "JOIN ARTICULO A ON L.CODARTICULO = A.CODIGO "
            "JOIN DOCCAB D ON D.CODIGO = L.CODDOCUMENTO AND D.TIPO=13 "
            "WHERE CAST(L.DESCUENTOS AS REAL) > 20 "
    ...
    },
    {
        ...
        "sql": (
            ...
            "ROUND(ROUND(SUM(CASE WHEN TIPO=1 THEN IMPORTE ELSE 0 END),2) / NULLIF(ROUND(SUM(CASE WHEN TIPO=2 THEN IMPORTE ELSE 0 END),2),0), 2) AS RATIO_COBROS_PAGOS "
            "FROM CAJA"
        ),
        "dept": [Dept.FINANZAS, Dept.DIRECCION],
        "rol": [Rol.DIRECTOR, Rol.GERENTE],
    ...
    },
    {
        ...
        "sql": (
            "SELECT COALESCE(C.NOMBRECOMERCIAL, C.RAZONSOCIAL, 'Sin nombre'), C.TEL, COUNT(D.CODIGO) AS N_SATS, "
            "ROUND(SUM(D.IMPORTETOTAL),2) AS COSTE_TOTAL_SAT "
            "FROM DOCCAB D JOIN CLIENTE C ON C.CODIGO=D.CODCLIENTE "
            "WHERE D.TIPO=2 GROUP BY C.CODIGO, C.TEL "
    ...
    },
    {
        ...
        "sql": (
            ...
            "ROUND(SUM(CAST(L.CANTIDAD AS REAL)*CAST(L.PRECIO AS REAL)),2) AS COSTE_TOTAL "
            "FROM DOCLIN L "
            "JOIN ARTICULO A ON L.CODARTICULO = A.CODIGO "
    ...
    },
    {
        ...
        "sql": (
            ...
            "FROM DOCCAB D JOIN CLIENTE C ON C.CODIGO=D.CODCLIENTE "
            "WHERE D.TIPO=2 AND D.ESTADOPEND=0 AND D.FECHA < date('now','-5 days') "
...

    {
        ...
        "sql": (
            ...
            "ROUND(SUM(CAST(L.CANTIDAD AS REAL)*CAST(L.PRECIO AS REAL))*100.0/(SELECT SUM(CAST(CANTIDAD AS REAL)*CAST(PRECIOVENTA AS REAL)) FROM DOCLIN),1) AS PCT_VENTAS "
            "FROM DOCLIN L "
            "JOIN ARTICULO A ON L.CODARTICULO = A.CODIGO "
            "JOIN DOCCAB D ON D.CODIGO = L.CODDOCUMENTO AND D.TIPO=13 "
    ...
    },
    {
        ...
        "sql": (
            "SELECT A.NOMBRE, A.PRECIOVENTA, A.STOCKARTICULO, COALESCE(F.NOMBRE,'Sin familia') AS FAMILIA "
            "FROM ARTICULO A LEFT JOIN FAMILIA F ON F.CODIGO=A.CODFAMILIA "
            "WHERE A.CODIGO NOT IN (SELECT DISTINCT CAST(CODARTICULO AS INTEGER) FROM DOCLIN) "
            "ORDER BY A.PRECIOVENTA DESC LIMIT 20"
        ),
    ...
    },
    {
        ...
        "sql": (
            "SELECT A.NOMBRE, ROUND(SUM(CAST(L.CANTIDAD AS REAL)*CAST(L.PRECIO AS REAL)),2) AS TOTAL_VENTAS, "
            "ROUND(SUM(CAST(L.CANTIDAD AS REAL)*CAST(L.PRECIO AS REAL))*100.0/(SELECT SUM(CAST(CANTIDAD AS REAL)*CAST(PRECIOVENTA AS REAL)) FROM DOCLIN "
...

    {
        ...
        "sql": (
            "SELECT 'Facturacion total' AS INDICADOR, CAST(COUNT(*) AS VARCHAR(20)) AS VALOR FROM DOCCAB WHERE TIPO=13 AND 1=0 "
            "UNION ALL SELECT 'N facturas', CAST(COUNT(*) AS VARCHAR(20)) FROM DOCCAB WHERE TIPO=13 "
            "UNION ALL SELECT 'Clientes activos', CAST(COUNT(DISTINCT CODCLIENTE) AS VARCHAR(20)) FROM DOCCAB WHERE TIPO=13 "
            "UNION ALL SELECT 'Presupuestos abiertos', CAST(COUNT(*) AS VARCHAR(20)) FROM DOCCAB WHERE TIPO=0 "
            "UNION ALL SELECT 'Albaranes pendientes', CAST(COUNT(*) AS VARCHAR(20)) FROM DOCCAB WHERE TIPO=11 "
            "UNION ALL SELECT 'SATs abiertos', CAST(COUNT(*) AS VARCHAR(20)) FROM DOCCAB WHERE TIPO=2 AND ESTADOPEND=0 "
            "UNION ALL SELECT 'Articulos sin stock', CAST(COUNT(*) AS VARCHAR(20)) FROM ARTICULO WHERE STOCKARTICULO <= 0 "
            "UNION ALL SELECT 'Articulos stock bajo', CAST(COUNT(*) AS VARCHAR(20)) FROM ARTICULO WHERE STOCKARTICULO > 0 AND STOCKARTICULO < 3 "
            "UNION ALL SELECT 'Total clientes', CAST(COUNT(*) AS VARCHAR(20)) FROM CLIENTE"
        ),
...

    {
        ...
        "sql": (
            "SELECT COALESCE(C.NOMBRECOMERCIAL, C.RAZONSOCIAL, 'Sin nombre') AS CLIENTE, C.TEL, "
            "COUNT(*) AS N_EFECTOS, "
            "ROUND(SUM(E.IMPORTE),2) AS DEUDA_VENCIDA "
            "FROM RECIBO1 E "
            "JOIN CLIENTE C ON C.CODIGO = E.CODPAGA "
            "WHERE E.FECHAVENC < date('now') AND (E.ACTIVO IS NULL OR E.ACTIVO != 1) "
            "GROUP BY C.CODIGO, C.TEL "
...

    {
        ...
        "desc_tecnica": (
            "Consulta sobre RECIBO3 filtrando FECHAVENCIMIENTO entre hoy y +30 días, "
            "PAGADO=0. JOIN con PROVEED para nombre. "
            "Agrupado por proveedor y semana de vencimiento para planificación de tesorería "
            "en JDDC Climatización."
        ),
        "sql": (
            "SELECT COALESCE(P.NOMBRECOMERCIAL, P.RAZONSOCIAL, 'Sin nombre') AS PROVEED, "
            "E.FECHA AS FECHAVENC, "
            "ROUND(SUM(E.IMPORTE),2) AS IMPORTE_PAGAR "
            "FROM RECIBO3 E "
            "JOIN PROVEED P ON P.CODIGO = E.CODPAGADOR "
            "WHERE E.FECHA BETWEEN date('now') AND date('now', '+30 days') "
            "AND (E.DEVOLUCION IS NULL OR E.DEVOLUCION != '1') "
            "GROUP BY P.CODIGO, COALESCE(P.NOMBRECOMERCIAL, P.RAZONSOCIAL, 'Sin nombre'), E.FECHA "
            "ORDER BY E.FECHA"
        ),
...

    {
        ...
        "desc_tecnica": (
            "Filtra ARTICULO donde STOCKARTICULO < 0 y 0 > 0. "
        ...
        "sql": (
            ...
            "A.STOCKARTICULO, 3 AS STOCK_MINIMO, "
            "(3 - A.STOCKARTICULO) AS DEFICIT, "
...

    {
        ...
        "desc_tecnica": (
            ...
            "Filtra artículos con STOCKARTICULO > 0 y sin movimiento de venta reciente. "
            "Calcula valor inmovilizado (stock * precio coste). "
            "Permite identificar obsolescencia en el catálogo de JDDC Climatización."
        ),
        "sql": (
            "SELECT A.REFERENCIA, A.NOMBRE, A.STOCKARTICULO, "
            "ROUND(A.STOCKARTICULO * COALESCE(A.PRECIOCOSTE,0), 2) AS VALOR_INMOVILIZADO, "
            "COALESCE(F.NOMBRE,'Sin familia') AS FAMILIA "
            "FROM ARTICULO A "
            "LEFT JOIN FAMILIA F ON F.CODIGO = A.CODFAMILIA "
            "WHERE A.STOCKARTICULO > 0 "
...

    {
        ...
        "sql": (
            ...
            "WHERE D.TIPO = 6 "
            "AND D.CODIGO NOT IN (SELECT CODALMACEN FROM DOCCAB WHERE TIPO=13 AND CODALMACEN IS NOT NULL) "
...

    {
        ...
        "desc_tecnica": (
            ...
            "JOIN con PROVEED para nombre y condiciones. "
        ...
        "sql": (
            ...
            "ROUND(SUM(CAST(L.CANTIDAD AS REAL)*CAST(L.PRECIO AS REAL)),2) AS TOTAL_COMPRADO, "
            "ROUND(AVG(CAST(L.CANTIDAD AS REAL)*CAST(L.PRECIO AS REAL)),2) AS TICKET_MEDIO "
            "FROM DOCLIN L "
            "JOIN ARTICULO A ON A.CODIGO = L.CODARTICULO "
            "JOIN PROVEED P ON P.CODIGO = A.PROVEEDDEFECTO "
            "JOIN DOCCAB D ON D.CODIGO = L.CODDOCUMENTO AND D.TIPO = 13 "
            "GROUP BY P.CODIGO, COALESCE(P.NOMBRECOMERCIAL, P.RAZONSOCIAL, 'Sin nombre') "
...

    {
        ...
        "sql": (
            ...
            "FROM CLIENTE C "
            "JOIN DOCCAB D ON D.CODCLIENTE = C.CODIGO AND D.TIPO = 13 "
            "WHERE C.BAJA IS NULL OR C.BAJA = '' "
            "GROUP BY C.CODIGO, C.TEL "
...

    {
        ...
        "desc_tecnica": (
            ...
            "Filtra líneas donde PRECIOVENTA < PRECIOCOSTE (margen negativo). "
            "Agrupa por factura para ver el impacto total. "
            "Alerta crítica para el control de márgenes en JDDC Climatización."
        ),
        "sql": (
            "SELECT D.CODIGO AS FACTURA, D.FECHA, COALESCE(C.NOMBRECOMERCIAL, C.RAZONSOCIAL, 'Sin nombre') AS CLIENTE, "
            "ROUND(SUM(CAST(L.CANTIDAD AS REAL)*CAST(L.PRECIO AS REAL)),2) AS IMPORTE_VENTA, "
            "ROUND(SUM(L.CANTIDAD * COALESCE(A.PRECIOCOSTE,0)),2) AS COSTE_TOTAL, "
            "ROUND(SUM(CAST(L.CANTIDAD AS REAL)*CAST(L.PRECIO AS REAL)) - SUM(L.CANTIDAD * COALESCE(A.PRECIOCOSTE,0)),2) AS MARGEN "
            "FROM DOCCAB D "
            "JOIN CLIENTE C ON C.CODIGO = D.CODCLIENTE "
            "JOIN DOCLIN L ON L.CODDOCUMENTO = D.CODIGO "
            "JOIN ARTICULO A ON L.CODARTICULO = A.CODIGO "
            "WHERE D.TIPO = 13 AND D.FECHA >= date('now','-90 days') "
            "GROUP BY D.CODIGO, D.FECHA, COALESCE(C.NOMBRECOMERCIAL, C.RAZONSOCIAL, 'Sin nombre') "
            "HAVING MARGEN < 0 "
...

    {
        ...
        "sql": (
            ...
            "A.STOCKARTICULO, "
            "ROUND(A.STOCKARTICULO - (SUM(L.CANTIDAD) / 6.0 * 3), 1) AS STOCK_PREVISTO, "
            "CASE WHEN A.STOCKARTICULO < (SUM(L.CANTIDAD) / 6.0 * 3) "
            "     THEN 'REPONER' ELSE 'OK' END AS ESTADO "
            "FROM DOCLIN L "
            "JOIN DOCCAB D ON D.CODIGO = L.CODDOCUMENTO AND D.TIPO = 13 "
            "JOIN ARTICULO A ON L.CODARTICULO = A.CODIGO "
            "WHERE D.FECHA >= date('now', '-180 days') "
            "GROUP BY A.CODIGO, A.REFERENCIA, A.NOMBRE, A.STOCKARTICULO "
...
# ─── Integración con QUERY_LIBRARY_EXTENDED ──────────────────────────────────
# Las 1000 consultas extendidas se importan del paquete query_library/
# y se combinan con las originales para formar el catálogo completo.

def _get_combined_library() -> List[Dict[str, Any]]:
    """
    Devuelve QUERY_LIBRARY (originales) + QUERY_LIBRARY_EXTENDED (1000 nuevas).
    Import lazy para evitar importaciones circulares en el arranque.
    """
    try:
        from backend.modules.db_simulator.query_library import QUERY_LIBRARY_EXTENDED
        return list(QUERY_LIBRARY) + list(QUERY_LIBRARY_EXTENDED)
    except Exception:
        return list(QUERY_LIBRARY)


# ─── Funciones de acceso ──────────────────────────────────────────────────────

def get_all_queries() -> List[Dict[str, Any]]:
    """Devuelve todas las consultas de la biblioteca (originales + extendidas)."""
    return _get_combined_library()


def get_queries_by_dept(dept: str) -> List[Dict[str, Any]]:
    """Filtra consultas por departamento."""
    all_q = _get_combined_library()
    return [
        q for q in all_q
...
def get_queries_by_tipo(tipo: str) -> List[Dict[str, Any]]:
    ...
    return [q for q in _get_combined_library() if q.get("tipo") == tipo]


def get_queries_by_urgencia(urgencia: str) -> List[Dict[str, Any]]:
    """Filtra consultas por urgencia."""
    return [q for q in _get_combined_library() if q.get("urgencia") == urgencia]


def get_query_by_id(query_id: str) -> Dict[str, Any]:
    """Obtiene una consulta por su ID. Busca en originales y extendidas."""
    for q in _get_combined_library():
...
def search_queries(
    ...
    results = list(_get_combined_library())
    if dept:
        results = [
            q for q in results
            if dept in (q.get("dept") if isinstance(q.get("dept"), list) else [q.get("dept")])
...
def get_catalog_summary() -> dict:
    """Devuelve un resumen del catálogo: totales por dept, rol, tipo y urgencia."""
    from collections import Counter
    queries = _get_combined_library()
