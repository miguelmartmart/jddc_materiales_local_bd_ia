...
export class ChatModule {
    ...
    async loadFullHistory() {
        try {
            ...
            container.innerHTML = '<div style="padding:20px; text-align:center; color:#999;">Cargando historial...</div>';
            ...
            if (response.ok) {
                ...
                container.innerHTML = '<div style="color:red; padding:20px;">Error cargando historial</div>';
            }
        } catch (error) {
    ...
    renderFullHistorySessions(data) {
        ...
        if (data.length === 0) {
            container.innerHTML = '<div style="padding:20px; text-align:center; color:#999;">No hay historial</div>';
            return;
        }
        ...
        data.forEach((item, index) => {
            ...
            div.addEventListener('click', () => {
                // Highlight active
                document.querySelectorAll('.history-session-item').forEach(el => el.style.background = 'white');
    ...
    renderFullHistoryMessages(item) {
        ...
        item.messages.forEach(msg => {
            ...
            if (content.includes('```')) {
                content = content.replace(/```([\s\S]*?)```/g, '<pre style="background:#1e293b; color:#fff; padding:10px; border-radius:4px; overflow-x:auto;">$1</pre>');
            }
    ...
    async loadHistory() {
        try {
            ...
            if (sessions.length === 0) {
                listContainer.innerHTML = '<div style="text-align: center; color: #94a3b8; font-size: 0.9em; margin-top: 20px;">No hay historial</div>';
                return;
            }
    ...
    async loadSession(sessionId) {
        ...
        messagesArea.innerHTML = '<div style="text-align:center; padding:20px; color:#666;">Cargando conversación...</div>';
        ...
        try {
            ...
            if (data.messages) {
                ...
                data.messages.forEach(msg => {
                    ...
                    this.appendMessage(msg.role === 'user' ? CHAT_ROLES.USER : CHAT_ROLES.AI, content);
                });
            }
    ...
    appendMessage(role, text, id = null, images = null) {
        ...
        if (images && images.length > 0) {
            ...
            imgContainer.style.justifyContent = role === 'user' ? 'flex-end' : 'flex-start';
