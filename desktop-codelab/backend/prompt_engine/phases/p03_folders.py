...
class FoldersPhase(BasePhase):
    ...
    def render(self, ctx: PhaseContext) -> str:
        ...
        return (
            ...
            f"  Muestra el árbol propuesto. Di 'Antes de crear nada, voy a verificar que las rutas estén libres.'\n"
            f"  Propón [[NEXT_ACTION]] con `Test-Path -Path 'ruta1','ruta2','ruta3'...` label: `Verificar carpetas`.\n"
            f"**Paso 2 — CREACIÓN** (solo DESPUÉS de ver el resultado del Paso 1):\n"
            f"  True=ya existe, False=libre. Propón [[NEXT_ACTION]] con\n"
            f"  `New-Item -ItemType Directory -Force -Path ...` solo con las rutas que dieron False. Label: `Crear carpetas`.\n\n"
        )
